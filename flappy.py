...
import pygame 
from pygame.locals import * #impoting pygame module
 #global variables for the games
 
FPS=32
SCREENWIDTH=289
SCREENHEIGHT=511
SCREEN=pygame.display.set_mode((SCREENWIDTH,SCREENHEIGHT)) #WILL DISPLAY A SCREEN WITH GIVRN HEIGHT AND WIDTH
GROUND_Y= SCREENHEIGHT*0.8
GAME_SPRITES={}
GAME_SOUNDS={}
PLAYER='sprites/redbird-midflap.png'
...
def mainGame(): #main game function
    ...
    while True:    #game loop for the gameplay
        for event in pygame.event.get():
            if event.type == QUIT or (event.type== KEYDOWN and event.key == K_ESCAPE):
                pygame.quit()
                sys.exit()
        ...
        crashTest= isCollide(playerx,playery,upperPipes,lowerPipes)   #upon collision
        if crashTest:
            return
        ...
        SCREEN.blit(GAME_SPRITES['background'],(0,0))    #blitting the sprites
        for upperPipe , lowerPipe in zip(upperPipes,lowerPipes):
        ...
        for digit in myDigits:   #score display
...
def isCollide(playerx,playery,upperPipes,lowerPipes):         #die or collison function
    ...
    for pipe in lowerPipes:
        if (playery + GAME_SPRITES['player'].get_height() > pipe['y']) and abs(playerx - pipe['x']) < GAME_SPRITES['pipe'][0].get_width():
            ...
            return True
    return False
def getRandompipe():             #function for rand combination of pipe
...
if __name__ == "__main__":
    ...
    pygame.display.set_caption('FLAPPY BIRD')         #assigning different sprites
    ...
    GAME_SOUNDS['die']=pygame.mixer.Sound('audio/audio_die.ogg')              #assigning different sounds
    GAME_SOUNDS['hit']=pygame.mixer.Sound('audio/audio_hit.ogg')
    GAME_SOUNDS['point']=pygame.mixer.Sound('audio/audio_point.ogg')
    GAME_SOUNDS['swoosh']=pygame.mixer.Sound('audio/audio_swoosh.wav')
    GAME_SOUNDS['wing']=pygame.mixer.Sound('audio/audio_wing.ogg')
